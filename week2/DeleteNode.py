...
def delete_node(node_to_delete):
    ...
    '''
    if(node_to_delete.next is None or node_to_delete is None):
        assert (), "Exception"

    while node_to_delete.next is not None:
        node_to_delete.value = node_to_delete.next.value
        if( node_to_delete.next.next is None):
            break
        node_to_delete = node_to_delete.next
    node_to_delete.next = None
    '''

    x = node_to_delete.next
    if x:
        node_to_delete.next = x.next
        node_to_delete.value = x.value
